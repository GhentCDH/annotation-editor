...
const generate_local_vitest_configs_1 = require("./lib/generate-local-vitest-configs");
const inline_vitest_workspace_1 = require("./lib/inline-vitest-workspace");
...
async function migrateToVitest4(tree) {
    ...
    const { foundWorkspaceFiles } = (0, inline_vitest_workspace_1.inlineVitestWorkspaceFiles)(tree, unhandled);
    ...
    const generatedConfigs = foundWorkspaceFiles
        ? (0, generate_local_vitest_configs_1.generateLocalVitestConfigs)(tree)
        : [];
    ...
    if (generatedConfigs.length > 0) {
        nextSteps.push(`Generated a minimal vitest config for packages that run vitest via a package.json script but had no local config (${generatedConfigs.join(', ')}). Without one, Vitest 4 climbs to the root \`test.projects\` config and fails to start from those directories. Review them and add any test options the packages need. ` +
