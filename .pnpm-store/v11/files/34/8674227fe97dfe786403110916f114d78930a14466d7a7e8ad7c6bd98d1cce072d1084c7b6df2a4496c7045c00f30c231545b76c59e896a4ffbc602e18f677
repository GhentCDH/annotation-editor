...
exports.generateLocalVitestConfigs = generateLocalVitestConfigs;
 ...
 */
function generateLocalVitestConfigs(tree) {
    const created = [];
    ...
    const rootHasTsconfig = tree.exists('tsconfig.base.json') || tree.exists('tsconfig.json');
...
function processPackage(tree, packageJsonPath, rootHasTsconfig, created) {
    let packageJson;
    try {
    ...
    if (!scripts || typeof scripts !== 'object')
        return;
    if (!Object.values(scripts).some((script) => typeof script === 'string' && scriptInvokesVitest(script))) {
        return;
    }
    ...
    if (siblings.some((f) => (0, vitest_config_files_1.isVitestConfigFile)(f)))
        return;
    ...
    if (siblings.some((f) => (0, vitest_config_files_1.isVitestWorkspaceFile)(f)))
        return;
    const ext = rootHasTsconfig || siblings.some((f) => /^tsconfig(\..+)?\.json$/.test(f))
        ? '.ts'
        : '.mjs';
    const configPath = (0, devkit_1.joinPathFragments)(dir, `vitest.config${ext}`);
    tree.write(configPath, `import { defineConfig } from 'vitest/config';
 ...
 */
function scriptInvokesVitest(script) {
    return script.split(/&&|\|\||;|\|/).some((command) => {
        const tokens = command.trim().split(/\s+/);
        const vitestIndex = tokens.findIndex((token, i) => (token === 'vitest' || token.endsWith('/vitest')) &&
            tokens[i - 1] !== 'run' &&
