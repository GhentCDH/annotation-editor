...
// fsDir   = path on disk relative to docsPath (e.g. 'guide')
// linkDir = URL path used in sidebar links   (e.g. 'guide')
...
// fsDir   = filesystem path relative to docsPath (e.g. 'guide')
...
const copyMd = (
  from,
  to,
  { depth = Number.MAX_SAFE_INTEGER, readmeOnly = false } = {},
) => {
  const fromDir = from || '.';
  if (!fs.existsSync(fromDir)) {
    // eslint-disable-next-line no-console
    console.warn(`Skipping ${fromDir} (does not exist)`);
    return;
  }
  ...
  searchFiles.forEach((file) => {
    if (readmeOnly && !file.endsWith('README.md')) return;
